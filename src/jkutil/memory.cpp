...
namespace jkutil
{
	...
	void* aligned_malloc(std::size_t p_size, std::size_t p_alignment)
	{
		...
		const std::size_t size = p_size == 0 ? 1 : p_size;

		//The maximum additional size neede dto guarentee alignment and space for the alignment offset header.
		const std::size_t max_header_size = sizeof(align_header_t) + (alignment - 1);

		//The total memory to allocate.
		const std::size_t memory_size = size + max_header_size;
		...
		if (memory)
		{
			...
			std::size_t total_space = memory_size - sizeof(align_header_t);
			std::size_t remaining_space = total_space;
			...
			std::size_t header_size = reinterpret_cast<char*>(aligned_ptr) - reinterpret_cast<char*>(memory);
	...
	void aligned_free(void* p_ptr, std::size_t p_size)
	{
		...
		if (p_ptr)
		{
			...
			const std::size_t header_size = *header;
